...
export function Step2Role() {
  ...
  return (
    <div className="flex flex-col items-center w-full max-w-2xl mx-auto">
      <WordReveal className="text-3xl md:text-4xl font-bold text-brand-dark mb-4 text-center" text="Tell me about yourself." />
      <p className="text-xl text-brand-dark/70 mb-10 text-center">What role are you targeting?</p>
      <AnswerReveal>
      ...
      <form onSubmit={handleSubmit} className="w-full mb-8">
        <input 
          ...
          className="w-full p-5 bg-white/80 backdrop-blur-md border border-indigo-100 rounded-2xl text-xl text-brand-dark focus:outline-none focus:border-gray-900/50 focus:ring-1 focus:ring-purple-500 transition-all text-center placeholder:text-brand-dark/30"
          autoFocus
        />
      </form>
      ...
      <div className="flex flex-wrap justify-center gap-3 mb-12">
        {examples.map(ex => (
          <button
            ...
            className="px-4 py-2 rounded-full bg-white/80 backdrop-blur-md border border-indigo-100 text-brand-dark/70 hover:bg-indigo-100/50 hover:text-brand-dark transition-all text-sm"
          >
            {ex}
          </button>
        ))}
