...
export function Step6Interests() {
  ...
  return (
    <div className="flex flex-col items-center w-full max-w-2xl mx-auto">
      <WordReveal className="text-3xl md:text-4xl font-bold text-brand-dark mb-10 text-center" text="What type of opportunities interest you?" />
      <AnswerReveal>
      ...
      <div className="grid grid-cols-2 gap-4 w-full mb-10">
        {options.map(opt => (
          <button
            key={opt}
            onClick={() => select(opt)}
            className={`p-5 rounded-2xl border text-lg font-medium transition-all ${data.companyTypes.includes(opt) ? 'bg-indigo-100/50 border-gray-900/50 text-brand-dark' : 'bg-white/80 backdrop-blur-md border-indigo-100 hover:border-gray-900/30 text-gray-700'}`}
          >
            {opt}
          </button>
        ))}
