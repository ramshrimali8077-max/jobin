...
export function Pricing() {
  ...
  return (
    <section id="pricing" className="bg-white/80 backdrop-blur-md pt-16 sm:pt-20 lg:pt-32 pb-20 sm:pb-24 lg:pb-32 font-sans flex flex-col items-center">
      <div className="w-full max-w-[1440px] mx-auto px-5 sm:px-8 lg:px-12 flex flex-col items-center">
        ...
        <p className="text-[16px] text-brand-dark/60 mb-10 sm:mb-12 text-left md:text-center self-start md:self-center max-w-xl">
        ...
        <div className="bg-[#F5F5F5] rounded-full p-1 flex items-center mb-12 sm:mb-16 self-start md:self-center">
          <button 
            ...
            className={`px-4 py-1.5 rounded-full text-[14px] font-medium transition-all duration-300 ${!isAnnual ? 'bg-brand-dark text-white shadow-sm' : 'text-brand-dark/60 hover:text-brand-dark'}`}
          >
            Monthly
          </button>
          <button 
            onClick={() => setIsAnnual(true)}
            className={`px-4 py-1.5 rounded-full text-[14px] font-medium transition-all duration-300 flex items-center gap-2 ${isAnnual ? 'bg-brand-dark text-white shadow-sm' : 'text-brand-dark/60 hover:text-brand-dark'}`}
          >
            Annual
            <span className={`text-[10px] rounded px-1.5 py-0.5 transition-colors ${isAnnual ? 'bg-white/80 backdrop-blur-md text-brand-dark' : 'bg-gray-200 text-gray-700'}`}>
              Save 20%
            </span>
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 sm:gap-6 w-full">
          ...
          <div className="rounded-2xl bg-[#F5F5F5] p-7 sm:p-8 flex flex-col">
            <div className="text-[12px] font-semibold text-brand-dark/40 tracking-widest uppercase mb-3">Starter</div>
            ...
            <div className="flex flex-col gap-2.5 mb-10 flex-1">
              {["50 AI email drafts", "Gmail integration", "Basic tracking", "3 templates"].map(f => (
                <div key={f} className="flex items-center gap-3">
                  ...
                  <span className="text-[14px] text-brand-dark">{f}</span>
                </div>
              ))}
            </div>
            ...
            <button 
              ...
              className="group w-full bg-brand-dark text-white text-[13px] sm:text-[14px] font-medium rounded-full px-5 py-3 flex items-center justify-center gap-3 transition-colors duration-300"
            >
          ...
          {/* PRO */}
          <div className="rounded-2xl bg-brand-dark text-white p-7 sm:p-8 flex flex-col shadow-[0_8px_32px_rgba(0,0,0,0.12)] relative -mt-2 mb-2 sm:mt-0 sm:mb-0 transform md:-translate-y-4">
            <div className="flex items-center justify-between mb-3">
              <div className="text-[12px] font-semibold text-brand-dark/40 tracking-widest uppercase">Pro</div>
              <div className="bg-white/80 backdrop-blur-md text-brand-dark text-[10px] font-semibold rounded px-2 py-0.5">Most Popular</div>
            </div>
            <div className="text-[clamp(2rem,4vw,3rem)] font-bold text-white leading-none mb-4">${isAnnual ? '15' : '19'} <span className="text-[16px] font-medium text-brand-dark/40">/ mo</span></div>
            <p className="text-[14px] text-brand-dark/40 mb-8 min-h-[42px]">For active job seekers sending 5–20 applications per week.</p>
            ...
            <button 
              onClick={() => navigate('/onboarding')}
              className="group w-full bg-white/80 backdrop-blur-md text-brand-dark text-[13px] sm:text-[14px] font-medium rounded-full px-5 py-3 flex items-center justify-center gap-3 transition-colors duration-300"
            >
              <div className="overflow-hidden h-[20px] flex flex-col justify-start text-center relative w-full">
          ...
          {/* TEAM */}
          <div className="rounded-2xl bg-[#F5F5F5] p-7 sm:p-8 flex flex-col">
            <div className="text-[12px] font-semibold text-brand-dark/40 tracking-widest uppercase mb-3">Team</div>
